...
export const AdminStatsModal: React.FC<AdminStatsModalProps> = ({ visible, onClose }) => {
  ...
  const fetchSecurityStats = async () => {
    try {
      ...
      const [policyResponse, cspResponse] = await Promise.all([
        fetch(`${process.env.EXPO_PUBLIC_API_URL}/api/security/policy-info`, {
          headers: { 'Authorization': `Bearer ${token}` },
        }),
        fetch(`${process.env.EXPO_PUBLIC_API_URL}/api/security/csp-stats`, {
          headers: { 'Authorization': `Bearer ${token}` },
        })
      ]);
  ...
  const fetchCSPDetails = async () => {
    try {
      ...
      const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/api/security/csp-stats`, {
        headers: { 'Authorization': `Bearer ${token}` },
      });
  ...
  const promoteUser = async (email: string) => {
    try {
      const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/api/admin/promote-user`, {
        method: 'POST',
        headers: {
